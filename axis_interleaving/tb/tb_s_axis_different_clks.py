...
# when the slave axi stream inputs run at different clocks.
#
# =============================================================================
# Libraries
# =============================================================================
...
def get_init_data(dut, throughputs_ratio):
    ...
    all_data_channel3 = []
    all_data_channel4 = []
    for i in range(throughputs_ratio*CAPTURE_SIZE):
        # random integer data
...
async def start_sending_data_ch4(tb, channel_data, capture_size):
    ...
    cycles_between_data(tb, int(tb.dut.g_CH4_THROUGHPUT_MBPS))
    time_btw_data_ps = cycles_btw_data*tb.AXIL_CLK_PERIOD-tb.AXIL_CLK_PERIOD
    rand_delay = random.randint(0, 100)
    await Timer(time=rand_delay, units='ns')
    for i in range(capture_size):
...
async def start_receiving_data_output(tb, capture_size, channels_used):
    ...
    for i in range(channels_used):
        capture_size_acc = capture_size_acc + capture_size[i]

    for i in range(capture_size_acc):
...
class TB(object):
    def __init__(self, dut):
        ...
        self.S_AXIS_HIGHEST_CLK_PERIOD = int(1000000/int(dut.g_AXIS_INPUT_HIGHEST_FREQUENCY_MHZ))   # highest frequency
        self.S_AXIS_HIGHEST_CLK_PERIOD = 10 * int(round(self.S_AXIS_HIGHEST_CLK_PERIOD/10))         # in case the last digit is not odd
        self.s_axis_slowest_clk_period = int(dut.g_AXIS_INPUT_HIGHEST_FREQUENCY_MHZ) - 0.2*int(dut.g_AXIS_INPUT_HIGHEST_FREQUENCY_MHZ)  # frequency 0.2 times slower than the highest frequency
        self.s_axis_slowest_clk_period = int(1000000/self.s_axis_slowest_clk_period)   
        self.s_axis_slowest_clk_period = 10 * int(round(self.s_axis_slowest_clk_period/10))
        s0_axis_clk = Clock(self.dut.s0_axis_aclk, self.S_AXIS_HIGHEST_CLK_PERIOD, units='ps')
        cocotb.start_soon(s0_axis_clk.start(start_high=False))
        s1_axis_clk = Clock(self.dut.s1_axis_aclk, self.s_axis_slowest_clk_period, units='ps')
        cocotb.start_soon(s1_axis_clk.start(start_high=False))
        s2_axis_clk = Clock(self.dut.s2_axis_aclk, self.S_AXIS_HIGHEST_CLK_PERIOD, units='ps')
        cocotb.start_soon(s2_axis_clk.start(start_high=False))
        s3_axis_clk = Clock(self.dut.s3_axis_aclk, self.s_axis_slowest_clk_period, units='ps')
        cocotb.start_soon(s3_axis_clk.start(start_high=False))
        s4_axis_clk = Clock(self.dut.s4_axis_aclk, self.S_AXIS_HIGHEST_CLK_PERIOD, units='ps')
...
async def different_frequencies(dut):
    """
        Data arrives to the inputs in the same rising edge and at the highest frequency in channels 0, 2, and 4.
        Data arrives to the inputs in the same rising edge, but with lower throughput and at the slowest frequency in channels 1 and 3.
    ...
    # ratio between the highest and lowest throughputs
    throughputs_ratio = 1 + int(int(dut.g_CH0_THROUGHPUT_MBPS)/int(dut.g_CH1_THROUGHPUT_MBPS))    
    cocotb.log.info("Ratio is: %d" % throughputs_ratio)   

    # set all channel status to OK
    await tb.set_channel_status_inputs(ENABLE_ALL_CHANNELS) 

    # init data
    data = get_init_data(dut, throughputs_ratio)
    ...
    capture_size_ch0 = throughputs_ratio*CAPTURE_SIZE
    capture_size_ch1 = CAPTURE_SIZE
    capture_size_ch2 = throughputs_ratio*CAPTURE_SIZE
    capture_size_ch3 = CAPTURE_SIZE
    capture_size_ch4 = throughputs_ratio*CAPTURE_SIZE
    cocotb.start_soon(start_sending_data_ch0(tb, channel_0, capture_size_ch0))
    cocotb.start_soon(start_sending_data_ch1(tb, channel_1, capture_size_ch1))
    cocotb.start_soon(start_sending_data_ch2(tb, channel_2, capture_size_ch2))
    cocotb.start_soon(start_sending_data_ch3(tb, channel_3, capture_size_ch3))
    cocotb.start_soon(start_sending_data_ch4(tb, channel_4, capture_size_ch4))
    capture_size = [capture_size_ch0, capture_size_ch1, capture_size_ch2, capture_size_ch3, capture_size_ch4]
