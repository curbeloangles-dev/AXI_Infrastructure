...
# 
# This testbench looks for testing the interleaving behaviour when data arrives
# at different rising edges to the slave axi stream inputs.
#
# =============================================================================
# Libraries
# =============================================================================
...
async def user_control_disabled(dut):
    """
        Data arrives to the inputs in different rising edges.
        All channels are set OK
    ...
    for j in range(CAPTURE_SIZE):
        ...
        if int(dut.g_CHANNELS_USED) >= 4:
            rdata = await tb.axis_sink.read() 
            output.append(bytes(rdata))
            expected_output.append(sdata_3)
        if int(dut.g_CHANNELS_USED) >= 5:
            rdata = await tb.axis_sink.read() 
            output.append(bytes(rdata))
            expected_output.append(sdata_4)
...
async def user_control_enabled(dut):
    """
        Data arrives to the inputs in different rising edges.
        All channel status are set OK.
        User takes control via registers.
        Output is compare with the expected output.
    ...
    # set all channels status to OK in registers
    await tb.axi_lite_master.write(CHANNEL_STATUS_OFFSET, bytes([ENABLE_ALL_CHANNELS]))
    
    # user takes control of the core via registers
