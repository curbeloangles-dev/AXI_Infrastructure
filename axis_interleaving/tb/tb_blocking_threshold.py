...
# status input changes its value and goes over or below the Blocking Threshold
# specified via generic.
#
# =============================================================================
# Libraries
# =============================================================================
...
async def blocking_threshold(dut):
    """
        ...
        Channel status of channel one is the only set OK.
        Output is compare with the expected output.
    ...
    for j in range(CAPTURE_SIZE):
        ...
        try:
            rdata = await with_timeout(tb.axis_sink.read(), 500, 'ns')
        except cocotb.result.SimTimeoutError:
            dut._log.info("No data was read at output!")

        if int(dut.g_CHANNELS_USED) - 1 < int(dut.g_BLOCKING_THRESHOLD):
            output.append(bytes(rdata))
