...
# 
# This testbench looks for testing the interleaving behaviour when user takes
# control of the core via registers.
#
# =============================================================================
# Libraries
# =============================================================================
...
class TB(object):
    ...
    def compare(self, a, b):
        ...
        for i in range(len(a)):
            self.dut._log.info("Iteration %d" % (i))
...
async def one_channel_enabled(dut):
    """
        ...
        Block 2:
            User takes control and enables one channel via registers.
            All channels are tested one by one.
            Output is compare with the expected output.
            
        Block 3:
            User control is disabled back.
            Output is compare with the expected output.
    ...
    ## BLOCK 2
    # one iteration per channel under user control
    enable_channel = [ENABLE_CHANNEL_0, ENABLE_CHANNEL_1, ENABLE_CHANNEL_2, ENABLE_CHANNEL_3, ENABLE_CHANNEL_4]
    for i in range(int(dut.g_CHANNELS_USED)):
        # enable a channel
        await tb.axi_lite_master.write(CHANNEL_STATUS_OFFSET, bytes([enable_channel[i]]))
        
        # user takes control of the core
        await tb.axi_lite_master.write(USER_CONTROL_OFFSET, bytes([ENABLE_USER_CONTROL]))
        ...
        # compare
        expected_output = data['channel_' + str(i)]
        tb.compare(output, expected_output)
        await Timer(15*tb.AXIL_CLK_PERIOD, units='ps')

    ## BLOCK 3
    # user leaves control of the core
    await tb.axi_lite_master.write(USER_CONTROL_OFFSET, bytes([DISABLE_USER_CONTROL]))
...
async def channel_zero_and_two_enabled(dut):
    """
        Data arrives to the inputs in the same rising edge.
        ...
        Block 2:
            User takes control and enables channels zero and two via registers.
            Output is compare with the expected output.
    ...
    await tb.axi_lite_master.write(CHANNEL_STATUS_OFFSET, bytes([ENABLE_CHANNEL_0_AND_2]))
...
async def all_channels_enabled(dut):
    """
        Data arrives to the inputs in the same rising edge.
        ...
        Block 2:
            User takes control and enables all channels via registers.
            Output is compare with the expected output.
    ...
    ## BLOCK 2
    # enable channels 0 and 2
    await tb.axi_lite_master.write(CHANNEL_STATUS_OFFSET, bytes([ENABLE_ALL_CHANNELS]))
